 ...
 * @version    SVN: $Id: functions.php 2689 2016-09-14 14:57:10Z oheil $
 */
		...
		if( isset($_SESSION['auto_update_new']) ) {
			...
			if( ini_get("allow_url_fopen")==1 ) {
				...
				if( preg_match("/Latest version is (.*)\R/",$news,$matches) ) {
					...
					if(	($old_v[0]==$new_v[0] && $old_v[1]==$new_v[1] && $old_v[2]<$new_v[2]) ||
...
function buildfolderlink($folder) {
    ...
    for ($i = 0; $i < count($elements); $i++) {
        ...
        echo "<a href=\"action.php?".NOCC_Session::getUrlGetSession()."&folder=" . $folderpath . "\">" . mb_convert_encoding($elements[$i], 'UTF-8', 'UTF7-IMAP') . "</a>";
    }
    echo "\n";
}
