<!-- start of $Id: prefs.php 2713 2017-09-06 12:34:03Z oheil $ -->
<?php
...
if (count($big_list) > 1) {
  for ($i = 0; $i < count($big_list); $i++) {
    if (!empty($currentInboxFolder) && $_SESSION['imap_namespace'] . $currentInboxFolder == $big_list[$i]) {
      array_push($inbox_folders_list, "\t<option value=\"".$big_list[$i]."\" selected=\"selected\">".mb_convert_encoding($big_list[$i], 'UTF-8', 'UTF7-IMAP')."</option>\n");
    } else {
      array_push($inbox_folders_list, "\t<option value=\"".$big_list[$i]."\">".mb_convert_encoding($big_list[$i], 'UTF-8', 'UTF7-IMAP')."</option>\n");
    }
  }
}
?>
    ...
    <fieldset>
      ...
      <table>
         <tr>
           ...
           <td class="prefsData">
             <?php if ($user_prefs->getSendHtmlMail() && file_exists('ckeditor.php')) {
...
<!-- end of $Id: prefs.php 2713 2017-09-06 12:34:03Z oheil $ -->
