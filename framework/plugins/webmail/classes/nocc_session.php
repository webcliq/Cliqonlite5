 ...
 * @version    SVN: $Id: nocc_session.php 2737 2017-12-14 14:42:35Z oheil $
 */
...
class NOCC_Session {
     ...
     * @static
     */
    public static function start($persistent=false) {
	global $conf;
	//does not work
	//ini_set('session.gc-maxlifetime',60*60*6);
	$cookie_lifetime=0;
	if( $persistent ) {
	...
	public static function check_session_age() {
		...
		if( isset($_SESSION['persistent']) && $_SESSION['persistent']==true ) {
		...
		if( isset($_SESSION['creation_time']) ) {
			...
			if( ! $session_expired && ( ! isset($_SESSION['persistent']) || $_SESSION['persistent']==false ) ) {
	 ...
	 * @static
	 */
	public static function new_session($persistent=false) {
		global $conf;
		$cookie_lifetime=0;
		if( $persistent ) {
			$cookie_lifetime=60*60*24*7*4; //4weeks
     ...
     * @static
     */
    public static function createCookie($persistent=false) {
	global $conf;
	$cookie_lifetime=0;
	if( $persistent ) {
